...
export const EXERCISES = [
  ...
  // ── 마지막 빈 축 — 팔 ──────────────────────────────────────
  //
  // `docs/07` 1장이 "빠진 축 셋"으로 유산소·균형·**팔**을 적었다. 앞의 둘은 불 끄기와
  // 돌다리가 채웠고 팔만 비어 있었다. 팝팝 클리커의 만세 동작이 그 자리다.
  //
  // EXP를 1로 둔 이유: 팔만 올리는 건 다리를 쓰는 동작보다 에너지 소모가 적다.
  // 좌우(1)와 같은 급으로 본다.
  {
    key: 'arm_raises', label: '팔', emoji: '🙌', unit: '회',
    group: 'upper', exp: 1,
    quality: '두 손목이 어깨 위로 올라갔다가 **내려와야** 1회. 든 채로 있으면 안 센다',
    safety: '어깨를 무리하게 젖히지 않는다. 팔을 곧게 위로만',
  },

  // ── FMS 세 번째 축 — 조작(manipulation) ──────────────────────
  //
  // `docs/07`이 말하는 기본운동기술 셋(이동·안정·조작) 중 마지막 축.
  // 풍선 팡팡·비눗방울 팡팡처럼 "화면 속 물체를 손으로 건드리는" 게임이
  // 처음으로 여기를 채운다 — 눈으로 보고 손을 뻗어 맞추는 협응이 지금까지
  // 없었다. 왼손/오른손을 나누는 이유는 두 기획서 다 "왼손 활동 횟수·
  // 오른손 활동 횟수"를 운동 데이터로 원해서다 — 한쪽 손만 쓰는 습관을
  // 부모가 알아챌 수 있게.
  {
    key: 'left_hand_hits', label: '왼손', emoji: '🫲', unit: '회',
    group: 'manipulate', exp: 1,
    quality: '왼손이 화면 속 오브젝트(풍선·비눗방울)에 실제로 닿아야 1회. 헛손질은 안 센다',
  },
  {
    key: 'right_hand_hits', label: '오른손', emoji: '🫱', unit: '회',
    group: 'manipulate', exp: 1,
    quality: '오른손이 화면 속 오브젝트(풍선·비눗방울)에 실제로 닿아야 1회. 헛손질은 안 센다',
  },
]
