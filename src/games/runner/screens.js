...
import { theme, img, ui, uiAt } from './theme.js';
...
export function showTitle() {
  return new Promise(resolve => {
    ...
    // 로고는 테마가 **가지고 있을 때만** 얹는다. 배경 그림에 로고가 이미 박혀 있는
    // 테마도 있어서(쥬라기 키비주얼), 없으면 조용히 건너뛰는 편이 맞다.
    const logo = theme().ui?.logo ? `<img class="title-logo" src="${ui('logo')}" alt="">` : '';
    const s = el(`
      <div class="screen title-bg">
        ${logo}
        <div class="title-start-wrap">
          <img class="start-img-btn" id="btn-start" data-pz-hit data-pz-dwell="1200" src="${ui('startButton')}" alt="START">
        </div>
      </div>`);
    overlay().appendChild(s);

    // 눌린 그림 — 4~8세는 "눌렀다"는 신호가 눈에 보여야 다시 누르지 않는다.
    const btn = s.querySelector('#btn-start');
    const pressed = theme().ui?.startButtonPressed ? ui('startButtonPressed') : null;
    if (pressed) {
      const down = () => { btn.src = pressed; };
        ...
        <div class="cam-body">
          ...
          <div class="cam-controls">
            ...
            <div class="gesture-hint hidden" id="gesture-hint">
              ${icon('hand')} 머리 위 <b>동그라미(O)</b> 3초 = 시작 · 팔로 <b>엑스(X)</b> = 이전 화면
        ...
        if (getLandmarks) {
          ...
          const applyBodyState = ok => {
            ...
            status.textContent = ok
              ? `${icon('check')} 좋아요! 준비 완료 — 이제 시작할 수 있어요`
    ...
    const s = el(`
      <div class="screen dark">
        ...
        <div class="gesture-hint hidden" id="tut1-gesture-hint">
          ${icon('hand')} 머리 위 <b>동그라미(O)</b>=건너뛰기 · 팔로 <b>엑스(X)</b>=이전 화면
...
export function showTutorial2(getPoseScore, quitCheck, isKeyboard, getLandmarks) {
  return new Promise(resolve => {
    clear();
    // 포즈 **종류**(lunge·forwardbend·armsopen)는 판정에 쓰는 엔진의 이름이다.
    // 사인판 그림과 실루엣만 테마에서 온다.
    const sp = theme().sprites;
    const poses = [
      { key: 'lunge', img: sp.obstacle.poseSign.lunge, sil: sp.poseSilhouette.lunge, label: '런지', keyLabel: 'A' },
      { key: 'forwardbend', img: sp.obstacle.poseSign.forwardbend, sil: sp.poseSilhouette.forwardbend, label: '상체 숙이기', keyLabel: 'S' },
      { key: 'armsopen', img: sp.obstacle.poseSign.armsopen, sil: sp.poseSilhouette.armsopen, label: '팔 벌리기', keyLabel: 'D' },
    ];
    const s = el(`
      <div class="screen dark">
        <h1>TUTORIAL</h1>
        <h2>COPY THE POSE — 사인판의 자세를 따라해요</h2>
        ${isKeyboard ? `<p class="kbd-hint">${icon('keyboard')} 키보드 모드: <b>A</b> · <b>S</b> · <b>D</b> 키를 누르고 있으면 해당 자세를 취한 걸로 인식돼요</p>` : ''}
        <div class="tut-row">
          ${poses.map(p => `
...
export function showCountdown() {
  return new Promise(resolve => {
    clear();
    const s = el(`<div class="screen"><img id="cd-img" style="height:40%" src="${uiAt('countdown', 3)}"></div>`);
    overlay().appendChild(s);
    const img = s.querySelector('#cd-img');
    const seq = [
      [uiAt('countdown', 3), 'countdown_beep'],
      [uiAt('countdown', 2), 'countdown_beep'],
      [uiAt('countdown', 1), 'countdown_beep'],
      [ui('startWord'), 'go'],
    ];
    let settled = false;
    let timer = null;
...
export function showLevelBanner(levelNum) {
  return new Promise(resolve => {
    ...
    const s = el(`<div class="screen"><img style="height:34%" src="${uiAt('levelComplete', levelNum)}"></div>`);
    overlay().appendChild(s);
    ...
    const s = el(`
      <div class="screen dark">
        ...
        <div class="gesture-hint hidden" id="go-gesture-hint">
          ${icon('hand')} 팔로 <b>엑스(X)</b>를 만들어 유지하면 종료돼요
...
export function showMissionComplete(stats) {
  return new Promise(resolve => {
    ...
    const s = el(`
      <div class="screen dark">
        <img style="height:22%" src="${ui('missionComplete')}">
        <img style="height:18%; margin-top:1%" src="${ui('podium')}">
        <div class="report">
          <table>
            <tr><td>${icon('star')} 획득한 별</td><td>${rec.score.stars}</td></tr>
            <tr><td>${icon('run')} 옆으로 피하기</td><td>${e.sideSteps}회</td></tr>
            <tr><td>${icon('zap')} 점프</td><td>${e.jumps}회</td></tr>
            <tr><td>${icon('down')} 앉았다 일어나기</td><td>${e.squats}회</td></tr>
            <tr><td>${icon('target')} 스트레칭 유지</td><td>${Math.round(poseTotal)}초</td></tr>
            <tr><td>${icon('timer')} 운동 시간</td><td>${Math.floor(rec.durationSec / 60)}분 ${rec.durationSec % 60}초</td></tr>
          </table>
    ...
    stats.save().then(r => {
      ...
      el.innerHTML = r.ok ? `${icon('check')} 운동 기록이 저장되었어요!` : '오프라인 — 기록은 나중에 자동 저장돼요';
    });
