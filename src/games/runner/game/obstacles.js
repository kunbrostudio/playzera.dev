...
// 장애물의 **종류**는 엔진이 안다(판정 방식이 다르므로). 그 종류를 어떤 그림으로
// 보여줄지는 테마가 정한다 — 우주에선 별 큐브, 정글에선 통나무일 수 있다.
const SPRITES = () => {
  const o = theme().sprites.obstacle;
  return {
    cube: { '-1': o.cubeLeft, '0': o.cubeCenter, '1': o.cubeRight },
    hurdleLow: o.hurdleLow,
    hurdleWide: o.hurdleWide,
    poseSign: o.poseSign,
    archGate: o.archGate,
  };
};
const HINTS = () => {
  const h = theme().sprites.hint;
  return { cube: null, hurdleLow: h.up, hurdleWide: h.down, poseSign: h.pose };
};
const LANES = [-1, 0, 1];
...
export class ObstacleRun {
  ...
  _drawHint(ctx, e) {
    ...
    if (e.type === 'cube') {
      ...
      name = theme().sprites.hint[e.hintLane > e.lane ? 'right' : 'left'];
    }
    const img = IMG[name];
    if (!img) return;
