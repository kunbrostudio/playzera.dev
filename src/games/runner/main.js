...
import { loadAssets, assetsReadyFor } from './assets.js';
import {
...
import { bindTouchPad } from './ui/touchPad.js';
...
// 에셋이 올라왔는지는 `assets.js`가 **테마 id로** 기억한다.
// 여기 불리언 하나로 두었더니 두 번째 테마가 로딩을 통째로 건너뛰었다 —
// 정글이 우주 하늘을 쓴 원인이다.
...
// `hud-level`·`hud-lives` 같은 안쪽 요소는 `ui/hud.js`가 직접 찾는다.
// 여기서 들고 있는 것은 **통째로 보이거나 숨기는** 두 개뿐이다.
let hudEl, hudLeftEl;
...
function setMenuOpen(open) {
  ...
  menuIco.src = menuOpen ? ui('menuClose') : ui('menu');
  updatePaused(); // 게임 화면에서 메뉴를 열면 잠깐 일시정지
}
function syncMenuIcons() {
  menuMusicImg.src = isBgmMuted() ? ui('musicOff') : ui('music');
  menuAudioImg.src = isSfxMuted() ? ui('sfxOff') : ui('sfxOn');
}
...
function updateHudDom() {
  // 그리는 일은 `ui/hud.js`가 한다 — 3D 러너와 **같은 코드**다.
  // 여기서 하는 것은 이 게임의 값을 그 모양에 맞춰 넘기는 것뿐이다.
  updateHud(document, {
    level: currentLevel + 1,
    stars: stats.stars,
    lives, maxLives: CONFIG.game.lives,
    jumps: stats.jumps, squats: stats.squats, sideSteps: stats.sideSteps,
  });
}
...
export async function boot() {
  ...
  // 잇는 것도 **3D 러너와 같은 코드**다(`ui/touchPad.js`). 게임마다 새로 짜면
  // `pointerleave`를 빠뜨려 손가락이 버튼 밖으로 나갔을 때 앉은 채로 굳는다.
  bindTouchPad(document, {
    left: actDodgeLeft,
    right: actDodgeRight,
    jump: actJump,
    duckStart: actDuckStart,
    duckEnd: actDuckEnd,
    poseDown: actPoseDown,
    poseUp: actPoseUp,
  }, signal);
  ...
  // ── 완전히 나가는 길 ★ ──
  // "게임 처음으로"는 이 게임의 타이틀까지만 간다(`doQuitConfirm` → `quitRequested`).
  // 허브로 나가려면 한 번 더 눌러야 하는데, 그걸 아이가 찾아 헤매지 않도록
  // 같은 창에 둔다. `doQuitConfirm`을 먼저 부르는 이유는 **저장 때문**이다 —
  // 해시부터 바꾸면 `onLeave`가 먼저 돌아 그 판의 운동이 저장 경로를 건너뛴다.
  ...
  ov.innerHTML = `<div class="screen dark"><h1>${theme().name}</h1><p id="load-p">로딩 중… 0%</p></div>`;
  if (!assetsReadyFor(theme().id)) {
    await loadAssets(f => {
