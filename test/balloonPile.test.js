...
describe('pileMetrics — 줄이 바구니 밖으로 안 나간다 ★', () => {
  it('실제로 자주 나올 개수(1~10)에서 줄 폭이 컨테이너를 안 넘는다', () => {
    // 겹침 상한(MAX_OVERLAP_PCT) 때문에 아주 많이 담기면(1부 레벨 3의
    // 목표 15개 전부) 줄이 살짝 넘칠 수 있다 — pile.js의 설계된 트레이드
    // 오프다("상한에 걸릴 만큼 많이 담았을 때는 줄이 조금 넘치는데,
    // 그때는 넘치게 두는 편이 낫다"). 여기서는 그 상한 밑에서 안 넘는
    // 것만 확인한다.
    for (let n = 1; n <= 10; n++) {
